...
typedef struct Position {
	...
	Position(Coordinate x = Coordinate(), size_t i = 0) : start(x), index(i) {
		if (ps.end < start) throw std::exception();
	}
	...
	int is_at_newline();
	Position& operator++();
	Position operator++(int) { Position tmp(*this); operator++(); return tmp; }
	...
	char get() { char res = cur(); ++(*this); return res; }
...
Position& Position::operator++() {
	if (!end_of_program()) {
		...
		if (at_newline == CHAR) {
			...
			start.inc_line(); index += at_newline;
		}
	}//else std::cout << "EOF\n";
	return *this;
}
